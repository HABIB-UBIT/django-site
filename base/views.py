...
def room(request,pk):
    ...
    room_messages= room.message_set.all()  ##_set.all() mostly use eith many one to relationships   ##### we are asking for all the messages of that specific room   ### We are intrested in getting all the messages of that specific room
    participants= room.participants.all()   ## .all() used with many to many relationships
    if request.method == 'POST':
        ...
        room.participants.add(request.user)  ## If the person writes the comment he will be directly become a participant
